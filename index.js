...
const LINK_PATTERN = /https?:\/\/www\.smogon\.com\/forums\/threads\/[^\s]+(?:\/page-\d+)?#post-(\d+)/i;

// Helper function for delay
const delay = ms => new Promise(resolve => setTimeout(resolve, ms));

// Helper function to detect page type (checks for "Remove")
...
// Run tests (if enabled) and Discord scanning
async function runTestsAndScan() {
  console.log(`Starting Smogon liking bot at ${new Date().toLocaleString()}`);
  console.log(`Configuration: SKIP_ALREADY_LIKED=${SKIP_ALREADY_LIKED}, HEADLESS=${HEADLESS}, COOLDOWN_MS=${COOLDOWN_MS}, RUN_TESTS=${RUN_TESTS}`);
  ...
  if (RUN_TESTS) {
    ...
    if (!step1) {
      console.log('Step 1 failed - stopping execution');
      return;
    }
    
    const step2 = await testStep2ButtonDetection();
    if (!step2) {
      console.log('Step 2 failed - stopping execution');
      return;
    }
    
    const step3 = await testStep3ButtonClick();
    if (!step3) {
      console.log('Step 3 failed - stopping execution');
      return;
    }
    
    console.log('\nAll tests completed successfully!');
  } else {
    console.log('\nSkipping tests (RUN_TESTS=false)');
  }
  
  console.log('\nStarting Discord scanning...\n');
  ...
  client.on('ready', async () => {
    ...
    if (!channel) {
      console.log('Channel not found! Check CHANNEL_ID and permissions.');
      return;
    }
    
    console.log(`Scanning history in channel: ${channel.name}`);
    const messages = await channel.messages.fetch({ limit: 100, force: true });
    console.log(`Found ${messages.size} messages`);
    
    for (const message of messages.values()) {
      console.log(`Message content: ${message.content}`);
      const matches = message.content.match(LINK_PATTERN);
      console.log(`Matches for message ${message.id}:`, matches);
      if (matches && matches[1]) {
        const postId = matches[1];
        const likeUrl = `https://www.smogon.com/forums/posts/${postId}/react?reaction_id=1`;
        console.log(`Processing: ${likeUrl}`);
  ...
  client.on('messageCreate', async (message) => {
    if (message.channelId === CHANNEL_ID) {
      ...
      const matches = message.content.match(LINK_PATTERN);
      console.log(`Matches for new message ${message.id}:`, matches);
      if (matches && matches[1]) {
        const postId = matches[1];
        const likeUrl = `https://www.smogon.com/forums/posts/${postId}/react?reaction_id=1`;
        console.log(`Processing new post: ${likeUrl}`);
        await likePost(likeUrl);
      }
    }
  });
